...
|---|---|---|---|
| `serviceKey`/`MobileOS`/`MobileApp`/`_type` | 🔴/🔴/🔴/⚪ | 위와 동일 | 고정값 동일 |
| `numOfRows` | ⚪ | 한 페이지 결과 수 | `20` (전국 17개 시도가 다 나오도록 넉넉히) |
| `pageNo` | ⚪ | 페이지 번호 | `1` |
| `lDongRegnCd` | ⚪ | 시도코드 — **생략하면 전국 시도 목록**, 넣으면 그 시도의 시군구 목록 | **넣지 않음** (전국 시도 17개를 받기 위해 의도적으로 생략) |
| `lDongListYn` | ⚪ | 전체 법정동코드 목록조회 여부 | **사용 안 함** (기본 동작으로 시도 목록만 필요하기 때문) |
